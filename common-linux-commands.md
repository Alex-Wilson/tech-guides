...
**Command Line Interface (CLI):** A command line interface, an alternative to the GUI, is also a way to interact with a computer or program. Instead of manipulating graphical elements like toggles, sliders, and buttons, the command line almost exlusively uses commands sent from the keyboard. Consider the use case for muting the audio on a computer. In a GUI enviornment (like windows) you likely click the speaker icon with the mouse, then move the "volume" slider to the bottom, or press the "mute" button. Alternatively, in a command line enviornment, you would use a command like "wpctl set-mute @DEFAULT_AUDIO_SINK@ 1". Your first thought is likely "this is really stupid", hitting the mute button is faster. Often times for simpler tasks the GUI will win in efficiency but not all tasks are the same. The command line is a vital tool that allows for explicit and extensible action over the computer or program. What if you want to mute all tabs exccept one.  
