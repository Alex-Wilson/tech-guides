...
Command Line Interface (CLI): A command line interface is simply a way to interact with a program. Instead of manipulating graphical elements, text based of keyboard based commands are sent directly to the computer (technically another program). Instead of a muting the audio with a button or knob the user might type "volume -100%". This is not an actual command but its simmilar to how the CLI is different. This might seem like an archaic and difficult way to interact with the computer but it is very explicit, well defined, and programmable. Most computers use this type of interface under the hood anyway.  
